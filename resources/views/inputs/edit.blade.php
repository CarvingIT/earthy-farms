    ...
    <div class="py-10 bg-[#f5f5f7] min-h-screen">
        <div class="max-w-xl mx-auto px-4 sm:px-6 space-y-6">
            ...
            <div>
                ...
                <h1 class="text-2xl font-bold tracking-tight text-neutral-900 mt-3">Edit Input Item</h1>
                <p class="text-xs text-neutral-500 mt-0.5">Update input details.</p>
            </div>

            <div class="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm">
                <form method="POST" action="{{ route('inputs.update', $input) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input id="name" type="text" name="name" value="{{ old('name', $input->name) }}" required autofocus
                    ...
                    <div>
                        ...
                        <input id="unit" type="text" name="unit" value="{{ old('unit', $input->unit) }}" required
                               class="mt-1.5 block w-full rounded-xl border-neutral-200 shadow-sm text-sm focus:border-neutral-500 focus:ring-neutral-500" />
                        @error('unit')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
