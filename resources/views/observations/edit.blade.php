    ...
    <div class="py-10 bg-[#f5f5f7] min-h-screen">
        <div class="max-w-xl mx-auto px-4 sm:px-6 space-y-6">
            ...
            <div>
                ...
                <h1 class="text-2xl font-bold tracking-tight text-neutral-900 mt-3">Edit Observation</h1>
                <p class="text-xs text-neutral-500 mt-0.5">Update field notes or growth milestones.</p>
            </div>

            <div class="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm"
                 x-data="{
                    farmers: {{ $farmers->toJson() }},
                    selectedFarmerId: '{{ old('farmer_id', $observation->crop->plot->farmer_id) }}',
                    plots: [],
                    selectedPlotId: '{{ old('plot_id', $observation->crop->plot_id) }}',
                    crops: [],
                    selectedCropId: '{{ old('crop_id', $observation->crop_id) }}',
                    init() {
                ...
                <form method="POST" action="{{ route('observations.update', $observation) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input id="observation_date" type="date" name="observation_date" value="{{ old('observation_date', $observation->observation_date) }}" required
                               class="mt-1.5 block w-full rounded-xl border-neutral-200 shadow-sm text-sm focus:border-neutral-500 focus:ring-neutral-500" />
