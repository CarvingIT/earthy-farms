    ...
    <div class="py-10 bg-[#f5f5f7] min-h-screen">
        <div class="max-w-xl mx-auto px-4 sm:px-6 space-y-6">
            ...
            <div>
                ...
                <h1 class="text-2xl font-bold tracking-tight text-neutral-900 mt-3">Edit Soil Report</h1>
                <p class="text-xs text-neutral-500 mt-0.5">Update soil report analysis results.</p>
            </div>

            <div class="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm"
                 x-data="{
                    farmers: {{ $farmers->toJson() }},
                    selectedFarmerId: '{{ old('farmer_id', $soilReport->crop->plot->farmer_id) }}',
                    plots: [],
                    selectedPlotId: '{{ old('plot_id', $soilReport->crop->plot_id) }}',
                    crops: [],
                    selectedCropId: '{{ old('crop_id', $soilReport->crop_id) }}',
                    init() {
                ...
                <form method="POST" action="{{ route('soil-reports.update', $soilReport) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-neutral-50 p-4 rounded-xl border border-neutral-100">
                        <div>
                            <label class="block text-[10px] font-bold text-neutral-500 uppercase tracking-wider">Farmer</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-bold text-neutral-500 uppercase tracking-wider">Plot</label>
                    ...
                    <div>
                        ...
                        <input id="sample_date" type="date" name="sample_date" value="{{ old('sample_date', $soilReport->sample_date) }}" required
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input id="Ph" type="number" step="0.1" name="Ph" value="{{ old('Ph', $soilReport->Ph) }}" required
                            ...
                            <div>
                                ...
                                <input id="EC" type="number" step="0.01" name="EC" value="{{ old('EC', $soilReport->EC) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>
                            ...
                            <div>
                                ...
                                <input id="OC" type="number" step="0.01" name="OC" value="{{ old('OC', $soilReport->OC) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>

                            <!-- N -->
                            <div>
                                <label for="N" class="block text-[11px] text-neutral-500">Nitrogen (N)</label>
                                <input id="N" type="number" step="0.1" name="N" value="{{ old('N', $soilReport->N) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>

                            <!-- P -->
                            <div>
                                <label for="P" class="block text-[11px] text-neutral-500">Phosphorus (P)</label>
                                <input id="P" type="number" step="0.1" name="P" value="{{ old('P', $soilReport->P) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>

                            <!-- K -->
                            <div>
                                <label for="K" class="block text-[11px] text-neutral-500">Potassium (K)</label>
                                <input id="K" type="number" step="0.1" name="K" value="{{ old('K', $soilReport->K) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>

                            <!-- Boron -->
                            <div>
                                <label for="Boron" class="block text-[11px] text-neutral-500">Boron</label>
                                <input id="Boron" type="number" step="0.01" name="Boron" value="{{ old('Boron', $soilReport->Boron) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>

                            <!-- Fe -->
                            <div>
                                <label for="Fe" class="block text-[11px] text-neutral-500">Iron (Fe)</label>
                                <input id="Fe" type="number" step="0.01" name="Fe" value="{{ old('Fe', $soilReport->Fe) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>

                            <!-- Zn -->
                            <div>
                                <label for="Zn" class="block text-[11px] text-neutral-500">Zinc (Zn)</label>
                                <input id="Zn" type="number" step="0.01" name="Zn" value="{{ old('Zn', $soilReport->Zn) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>

                            <!-- Cu -->
                            <div>
                                <label for="Cu" class="block text-[11px] text-neutral-500">Copper (Cu)</label>
                                <input id="Cu" type="number" step="0.01" name="Cu" value="{{ old('Cu', $soilReport->Cu) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>

                            <!-- Mg -->
                            <div>
                                <label for="Mg" class="block text-[11px] text-neutral-500">Magnesium (Mg)</label>
                                <input id="Mg" type="number" step="0.01" name="Mg" value="{{ old('Mg', $soilReport->Mg) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>

                            <!-- S -->
                            <div>
                                <label for="S" class="block text-[11px] text-neutral-500">Sulfur (S)</label>
                                <input id="S" type="number" step="0.01" name="S" value="{{ old('S', $soilReport->S) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>
                            ...
                            <div class="col-span-2 sm:col-span-3">
                                ...
                                <input id="microbial_count" type="number" name="microbial_count" value="{{ old('microbial_count', $soilReport->microbial_count) }}" required
                                       class="mt-1 block w-full rounded-lg border-neutral-200 shadow-sm text-xs focus:border-neutral-500 focus:ring-neutral-500" />
                            </div>
                        </div>
                    </div>
