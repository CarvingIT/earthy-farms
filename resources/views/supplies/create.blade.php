    ...
    <div class="py-10 bg-[#f5f5f7] min-h-screen">
        <div class="max-w-xl mx-auto px-4 sm:px-6 space-y-6">
            ...
            <div>
                ...
                <h1 class="text-2xl font-bold tracking-tight text-neutral-900 mt-3">Apply Supply / Input</h1>
                <p class="text-xs text-neutral-500 mt-0.5">Record the quantity of raw material applied to an active crop.</p>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm"
                ...
                <form method="POST" action="{{ route('supplies.store') }}" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select id="input_id" name="input_id" required
                            ...
                            @foreach($inputs as $input)
                                <option value="{{ $input->id }}" {{ old('input_id') == $input->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input id="quantity" type="number" step="0.01" name="quantity" value="{{ old('quantity') }}" required placeholder="e.g. 50"
                               class="mt-1.5 block w-full rounded-xl border-neutral-200 shadow-sm text-sm focus:border-neutral-500 focus:ring-neutral-500" />
                        @error('quantity')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input id="loading_date" type="date" name="loading_date" value="{{ old('loading_date', date('Y-m-d')) }}" required
                               class="mt-1.5 block w-full rounded-xl border-neutral-200 shadow-sm text-sm focus:border-neutral-500 focus:ring-neutral-500" />
                        @error('loading_date')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea id="note" name="note" rows="3" placeholder="e.g. Applied near crop roots after light weeding"
                                  class="mt-1.5 block w-full rounded-xl border-neutral-200 shadow-sm text-sm focus:border-neutral-500 focus:ring-neutral-500">{{ old('note') }}</textarea>
                        @error('note')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
