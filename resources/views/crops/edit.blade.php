    ...
    <div class="py-10 bg-[#f5f5f7] min-h-screen">
        <div class="max-w-xl mx-auto px-4 sm:px-6 space-y-6">
            ...
            <div>
                ...
                <h1 class="text-2xl font-bold tracking-tight text-neutral-900 mt-3">Edit Crop</h1>
                <p class="text-xs text-neutral-500 mt-0.5">Update crop details.</p>
            </div>

            <div class="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm" 
                 x-data="{
                    farmers: {{ $farmers->toJson() }},
                    selectedFarmerId: '{{ old('farmer_id', $crop->plot->farmer_id) }}',
                    plots: [],
                    selectedPlotId: '{{ old('plot_id', $crop->plot_id) }}',
                    init() {
                ...
                <form method="POST" action="{{ route('crops.update', $crop) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input id="name" type="text" name="name" value="{{ old('name', $crop->name) }}" required
                               class="mt-1.5 block w-full rounded-xl border-neutral-200 shadow-sm text-sm focus:border-neutral-500 focus:ring-neutral-500" />
                        @error('name')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input id="variety" type="text" name="variety" value="{{ old('variety', $crop->variety) }}" required
                               class="mt-1.5 block w-full rounded-xl border-neutral-200 shadow-sm text-sm focus:border-neutral-500 focus:ring-neutral-500" />
                        @error('variety')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input id="sowing_date" type="date" name="sowing_date" value="{{ old('sowing_date', $crop->sowing_date) }}" required
                               class="mt-1.5 block w-full rounded-xl border-neutral-200 shadow-sm text-sm focus:border-neutral-500 focus:ring-neutral-500" />
                        @error('sowing_date')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input id="harvest_date" type="date" name="harvest_date" value="{{ old('harvest_date', $crop->harvest_date) }}" required
                               class="mt-1.5 block w-full rounded-xl border-neutral-200 shadow-sm text-sm focus:border-neutral-500 focus:ring-neutral-500" />
                        @error('harvest_date')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
